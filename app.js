...
window.markItem = async function(itemId, status) {
  ...
  if (status === 'missing') {
    ...
    if (orderData) {
      ...
      await db.from('profiles').update({ wallet_balance: db.raw(`wallet_balance + ${price}`) }).eq('id', orderData.customer_id);
      await db.from('wallet_transactions').insert({ user_id: orderData.customer_id, order_id: State.currentOrder.id, type: 'refund', amount: price, description: `Refund: ${item.product_name}` });
    }
...
window.completeDelivery = async function(orderId) {
  ...
  if (order) {
    ...
    await db.from('profiles').update({ wallet_balance: db.raw(`wallet_balance + ${driverEarning}`) }).eq('id', order.driver_id);
    await db.from('wallet_transactions').insert({ user_id: order.driver_id, order_id: orderId, type: 'earning', amount: driverEarning, description: 'Delivery completed' });
  }
